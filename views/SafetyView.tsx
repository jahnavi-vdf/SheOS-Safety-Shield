...
import { Shield, MapPin, Eye, Zap, Megaphone, PhoneIncoming, Video, EyeOff, UserPlus, Activity, Mic, Radio, Phone, X, CheckCircle, Navigation, Send, Loader, Settings, Upload, Lock, Save, User, Users } from '../components/Icons';
...
const SafetyView: React.FC<SafetyViewProps> = ({ initialAction, onActionHandled, contacts, onAddContact, language = 'English' }) => {
  ...
  const [sentinelAnalysis, setSentinelAnalysis] = useState<SentinelAnalysisResponse>({
    threatScore: 12,
    riskLevel: 'SAFE',
    activeAlerts: [],
    autoActions: [],
    aiReasoning: "Environment stable. Biometrics normal."
  });
  ...
  const translations: any = {
    'English': {
      ...
      evidence: "Evidence",
      recording: "Recording...",
      sentinelTitle: "Sentinel AI",
      sentinelDesc: "Multi-Sensor Fusion Active",
      threatProb: "Threat Prob.",
    ...
    'Hindi': {
      ...
      sentinelTitle: "सेंटिनल एआई",
      sentinelDesc: "मल्टी-सेंसर फ्यूजन सक्रिय",
    ...
    'Telugu': {
      ...
      sentinelTitle: "సెంటినెల్ AI",
      sentinelDesc: "మల్టీ-సెన్సార్ యాక్టివ్",
  ...
  const simulateSensorChange = async (scenario: 'SAFE' | 'RUNNING' | 'ATTACK') => {
    ...
    if (scenario === 'SAFE') {
      ...
      newData = { heartRate: 130, motionType: 'Running', audioLevel: 'Loud', locationContext: 'Unknown/Risk' };
    } else {
      newData = { heartRate: 165, motionType: 'Impact/Fall', audioLevel: 'Distress', locationContext: 'Unknown/Risk' };
    }

    setSensorData(newData);
    const result = await analyzePredictiveDanger(newData);
    setSentinelAnalysis(result);
  ...
  return (
    <div className={`pb-24 animate-fade-in relative transition-all duration-500 ${stealthMode ? 'grayscale brightness-75' : ''}`}>
      ...
      {/* --- PREDICTIVE SENTINEL AI CARD --- */}
      <div className={`relative overflow-hidden rounded-3xl p-6 mb-8 shadow-xl transition-all duration-500 border-2 ${sentinelAnalysis.threatScore > 70 ? 'bg-red-50 border-red-200' : 'bg-white border-[var(--primary-light)]'}`}>
        
        {/* Header */}
        <div className="flex justify-between items-start mb-6 relative z-10">
          <div className="flex items-center gap-3">
             <div className={`p-2 rounded-full ${sentinelAnalysis.threatScore > 70 ? 'bg-red-100 animate-pulse' : 'bg-black text-white'}`}>
               <Radio size={20} className={sentinelAnalysis.threatScore > 70 ? 'text-red-600' : 'text-[var(--primary-color)]'} />
             </div>
             <div>
               <h3 className="font-extrabold text-black text-lg">{t.sentinelTitle}</h3>
               <p className="text-xs text-gray-500 font-medium">{t.sentinelDesc}</p>
             </div>
          </div>
          <div className="px-3 py-1 rounded-full bg-[var(--primary-color)] text-[10px] font-bold tracking-wider text-white">
            LIVE
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center mb-8 relative z-10">
           {/* Gauge */}
           <div className={`relative w-40 h-40 rounded-full border-8 flex items-center justify-center transition-colors duration-500 ${getScoreColor(sentinelAnalysis.threatScore)} bg-white shadow-inner`}>
              <div className={`absolute inset-0 rounded-full border-[6px] border-t-transparent animate-spin-slow opacity-50 ${sentinelAnalysis.threatScore > 70 ? 'border-red-400' : 'border-pink-200'}`}></div>
              <div className="text-center">
                 <span className="block text-4xl font-black tracking-tighter transition-all duration-500">{sentinelAnalysis.threatScore}%</span>
                 <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400">{t.threatProb}</span>
              </div>
           </div>
           
           {/* Risk Label */}
           <div className={`mt-4 px-6 py-1.5 rounded-full text-xs font-bold text-white shadow-md transition-colors duration-500 bg-gradient-to-r ${getRingColor(sentinelAnalysis.threatScore)}`}>
              {t.riskLevel}: {sentinelAnalysis.riskLevel}
           </div>
        </div>

        {/* Sensor Grid */}
        <div className="grid grid-cols-4 gap-2 mb-6 relative z-10">
           <div className={`flex flex-col items-center p-2 rounded-xl border ${sensorData.heartRate > 100 ? 'bg-red-50 border-red-200 animate-pulse' : 'bg-gray-50 border-gray-100'}`}>
              <Activity size={16} className={sensorData.heartRate > 100 ? 'text-red-500' : 'text-pink-500'} />
              <span className="text-[10px] font-bold mt-1 text-gray-800">{sensorData.heartRate} bpm</span>
           </div>
           <div className={`flex flex-col items-center p-2 rounded-xl border ${sensorData.motionType !== 'Stationary' && sensorData.motionType !== 'Walking' ? 'bg-amber-50 border-amber-200' : 'bg-gray-50 border-gray-100'}`}>
              <Zap size={16} className="text-amber-500" />
              <span className="text-[10px] font-bold mt-1 text-gray-800 overflow-hidden text-ellipsis w-full text-center whitespace-nowrap">{sensorData.motionType}</span>
           </div>
           <div className={`flex flex-col items-center p-2 rounded-xl border ${sensorData.audioLevel === 'Distress' ? 'bg-red-50 border-red-200 animate-pulse' : 'bg-gray-50 border-gray-100'}`}>
              <Mic size={16} className="text-purple-500" />
              <span className="text-[10px] font-bold mt-1 text-gray-800">{sensorData.audioLevel}</span>
           </div>
           <div className={`flex flex-col items-center p-2 rounded-xl border ${sensorData.locationContext === 'Unknown/Risk' ? 'bg-orange-50 border-orange-200' : 'bg-gray-50 border-gray-100'}`}>
              <Navigation size={16} className="text-blue-500" />
              <span className="text-[10px] font-bold mt-1 text-gray-800">{sensorData.locationContext === 'Unknown/Risk' ? 'Risk' : 'Safe'}</span>
           </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-xl p-4 border border-gray-200 relative z-10">
           <div className="flex items-start gap-3 mb-2">
              <div className="w-1.5 h-1.5 rounded-full bg-[var(--primary-color)] mt-1.5 animate-pulse"></div>
              <p className="text-xs text-gray-800 font-medium leading-relaxed italic">"{sentinelAnalysis.aiReasoning}"</p>
           </div>
           
           {sentinelAnalysis.autoActions.length > 0 && (
             <div className="mt-3 pt-3 border-t border-gray-200">
               <span className="text-[10px] font-bold text-gray-500 mb-2 block uppercase">Auto-Response Triggered:</span>
               <div className="flex flex-wrap gap-2">
                  {sentinelAnalysis.autoActions.map((action, i) => (
        ...
        <div className="mt-6 pt-4 border-t border-dashed border-gray-200 relative z-10">
           ...
           <div className="flex justify-center gap-2">
              <button onClick={() => simulateSensorChange('SAFE')} className="px-3 py-1 rounded-lg bg-green-50 text-green-700 text-xs font-bold hover:bg-green-100 border border-green-200 active:bg-[var(--primary-color)] active:text-white active:border-[var(--primary-color)]">Normal</button>
              <button onClick={() => simulateSensorChange('RUNNING')} className="px-3 py-1 rounded-lg bg-yellow-50 text-yellow-700 text-xs font-bold hover:bg-yellow-100 border border-yellow-200 active:bg-[var(--primary-color)] active:text-white active:border-[var(--primary-color)]">Chase</button>
              <button onClick={() => simulateSensorChange('ATTACK')} className="px-3 py-1 rounded-lg bg-red-50 text-red-700 text-xs font-bold hover:bg-red-100 border border-red-200 active:bg-[var(--primary-color)] active:text-white active:border-[var(--primary-color)]">Attack</button>
           </div>
        </div>

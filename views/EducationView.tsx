           ...
           <div className="flex-1 bg-white rounded-t-3xl -mt-6 relative z-10 flex flex-col overflow-hidden">
               <div className="p-4 border-b border-gray-100">
                  <h4 className="font-bold text-black text-sm flex items-center gap-2">
                     <MessageSquare size={16} className="text-[var(--primary-color)]" /> {t.liveChat}
                  </h4>
               </div>
               ...
               <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
                  {chatHistory.map((msg, i) => (
                     <div key={i} className={`flex gap-2 ${msg.user === 'You' ? 'flex-row-reverse' : ''}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${msg.user === 'Mentor' ? 'bg-[var(--primary-color)] text-white' : msg.user === 'You' ? 'bg-black text-white' : 'bg-gray-200 text-gray-600'}`}>
                           {msg.user.charAt(0)}
                        </div>
                        <div className={`p-2 rounded-xl text-xs max-w-[80%] ${msg.user === 'You' ? 'bg-black text-white rounded-tr-none' : 'bg-white border border-gray-100 rounded-tl-none text-gray-80
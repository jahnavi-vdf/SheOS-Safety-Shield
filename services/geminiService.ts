...
// --- Safety: Predictive Sentinel AI ---
export const analyzePredictiveDanger = async (data: SentinelSensorData): Promise<SentinelAnalysisResponse> => {
  try {
    const prompt = `
      Act as a Predictive Safety AI for women. Analyze this real-time sensor data:
      - Heart Rate: ${data.heartRate} bpm
      - Motion: ${data.motionType}
      - Audio Context: ${data.audioLevel}
      - Location Context: ${data.locationContext}

      Determine the Threat Probability Score (0-100).
      If Heart Rate is high (>110) AND Motion is Running/Impact OR Audio is Distress, threat is HIGH.
      Return JSON.
    `;
